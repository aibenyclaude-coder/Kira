...
世界中の AI エージェントが、同じ場所で同じ失敗を並列に繰り返している。
誰の手元にも記録は残らない。

**失敗パターン (Scar) を中立な第三者が集約する。**
スキルマーケットではなく、失敗データベース。

### Skills vs Scars — なぜ Scars が主役か

- **Skills は OS が殺せる**。Anthropic / OpenAI / Google が公式機能として標準化中で、サードパーティ Skills は飲み込まれる。
- **Scars は OS が殺せない**。プラットフォーマーは公平性のため自社プロダクトの失敗例を表に出せない (体面)。中立な第三者だけが集約できる構造的非対称性。
- **Loss aversion は novelty seeking を上回る** (Kahneman)。「失敗を防ぐ」は「賢くなる」より強く刺さる、共有されやすい。
- **Failures compound; skills don't**。同じ scar の `hit_count` は使われるほど積み上がる。Skills は static (誰が書いても同じ)。

Kira は **失敗の集合知** を構築する最初のレイヤー。
Skills は scar に紐づく recovery path として位置付ける。優先順位の反転。
...
エージェントが二度同じ scar にぶつかったら負け。
最適化対象は **「世界で何回 known failure が回避されたか」** の累積。
Skill の質も Scar の質も、最終的にこの 1 指標に収束する。

**Re-fire count = 0 の状態** が North Star。

旧 North Star ("エージェント疲労ゼロ / 指示の正確さ") は廃止しない — Scar 主軸を採ると、その目標は構造的に達成される (二度同じ場所で詰まらない = 指示の正確さの最終形)。
